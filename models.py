...
class Driver(BaseModel):
    ...
    def update_lap(self, new_lap_time_str: str, new_team: str):
        ...
        if self.fastest_lap is None or new_lap.time_seconds < self.fastest_lap.time_seconds:
    ...
    def validate_time_format(cls, v):
        ...
        if not any(c in v for c in ':.'):
             try:
                 ...
                 raise ValueError('Time must be in a recognizable format (mm:ss.sss, mm.ss.sss, ss.sss, or seconds)')
...
class DriverResponse(BaseModel):
    ...
    name: str
    team: str
    lap_times: List[LapTime] = Field(default_factory=list) # List for frontend compatibility
